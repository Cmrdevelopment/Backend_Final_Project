...
const { CommentErrors, CommentSuccess } = require('../../helpers/jsonResponseMsgs');
...
const getAll = async (req, res, next) => {
    try {
      const allComments = await Comment.find().populate('users').populate('references');
      if (allComments) {
...
const getByReference = async (req, res, next) => {
    try {
      ...
      let comments;
      if (refType === "User") {
        comments = await Comment.find({ referenceUser: id }).populate(
          "users referenceComment"
        );
